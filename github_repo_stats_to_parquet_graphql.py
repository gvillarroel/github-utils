...
def estimate_rest_core_requests(repo_count: int, *, include_contributors: bool, include_stats: bool) -> int:
    per_repo = 0
    if include_contributors:
        per_repo += 1
    if include_stats:
        per_repo += 4
    return 1 + (repo_count * per_repo)
...
def main() -> int:
    ...
    for repo_summary, languages in normalized_repos:
        ...
        if repo_context.default_branch:
            try:
                repo_path = clone_repo(repo_context, workspace_dir or Path(), token)
    ...
    print(
        ...
        f"{estimate_rest_core_requests(len(repository_rows), include_contributors=args.include_contributors, include_stats=args.include_stats)}; "
        f"GraphQL remaining: {graphql_rate['remaining']}.",
        file=sys.stderr,
    )
    return 0
